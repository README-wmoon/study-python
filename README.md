...
인터프리터
    인터프리트를 해주는 프로그램 또는 명령어.(개별처리라 힘듬) -> 한줄한줄씩 해석 -> 컴파일이 낳다 -> 하지만 하나의 코드를 해석하는 것은 좋다.
...
2. Python 프로그램
    프로그램
    PVM(파이썬 가상 운영체제) : Python 프로그램을 OS에 맞게 번역한다.
    OS(운영체제): 하드웨어에 적절한 전기신호를 흘려주는 역할.
    하드웨어
...
표기법
    파스탈 표기법(클래스명, 오류명) - 파이썬
        대문자로 시작하고 이어지는 단어의 시작은 대문자로 작성한다.
        PascalCase

    카멜 표기법(Java 등에서 사용) - 자바
...
자료구조란? -> 의미없는 데이터를 의미있게 해주는것
알고리즘이란? -> 문제해결을 하려고 쓰인다.
...
알고리즘과 자료구조
    ...
    2. 자료구조란, 의미 없는 값을 하나의 정보로 변환하고 이는 저장공간의 종류를 의미한다.
        예) 3개의 정수가 있을 떄, 이를 빠르게 가져오는 서비스를 제작해야한다.
...
dict
    한 쌍으로 저장되어 관리한다. -> 두개의 자료구조로 저장이되는 원리 -> key: value
...
함수를 사용하는 목적
    1. 재사용
        절대 특정성을 부여해서는 안된다. -> 화면에서 할것과 서버에서 할것을 구분하여야 한다
...
클래스의 필드 사용
    1. 객체화(instance) -> 객체를 만드는 화를 객체화이고 -> 그 인스턴스안에 있는 변수를 인스턴스 변수이다 -> 객체랑 객체화는 다름
...
self
    ...
    이 때 접근한 객체가 가지고 있는 필드의 주소값이 self라는 변수에 자동으로 넘긴다.
...
모듈 사용 []는 코드에 쓰는게 아니다 -> 설명을 쓰는것이다.
        import = 결론이다, from = 요것만 들고오겠다
...
try, except 문
    ...
    1.
        ...
        except 발생오류(class) as 오류객체: -> 클래스를 객체화 한것이다.
            오류 발생 시 실행할 문장
...
파일 읽기
    기존 내용을 한 줄씩 읽어올 때 'r'을 작성하여 읽기 모드로 파일을 열어준다.
    open(path, 'r')
